...
$res_low = $db->select("config_data","COW","FAT_LOW_LIMIT");
$res_high = $db->select("config_data","COW","FAT_HIGH_LIMIT");
...
if (($getfile = fopen($csv_file, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($getfile, 1000, ",")) !== FALSE) {
        ...
        if($i > $row_low[3] && $i <= $row_high[3]){
            ...
            $db_up->insert($cow_fat_data,"cow_fat");
        }
        $i += 0.1;
        $fat1 += 0.1;
    }
