...
$res_low = $db->select("config_data","BUFFALO","SNF_LOW_LIMIT");
$res_high = $db->select("config_data","BUFFALO","SNF_HIGH_LIMIT");
...
//    $data = fgetcsv($getfile, 1000, ",");
    ...
    $fat = $row_low[3];
//    $fat = $row_high[3];
    $db_up->truncate("buffalo_fat_snf");
    $f_start = $fat1 * 10;
    $f_end = $fat2 * 10;
    $row = 1;
...
//            echo "<pre>";
//            print_r($a);
            $db_up->insert($a,"buffalo_fat_snf");
        }
        $fat1 = $fat1 + 0.1;
    }
    fclose($getfile);
